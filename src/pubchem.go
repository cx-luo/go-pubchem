	...
	for i := 0; i < len(s.Rows); i++ {
		...
		go func() {
			...
			err := func(c Compound) error {
				_, err := dao.AidbCursor.NamedExec(insertSql, c)
				if err != nil {
					return err
				}
...
func InsertCompoundInfo() {
	...
	//}
	err := dao.AidbCursor.Select(&cids, `select a.cid from enotess.buchwald_reactants_all a left join ai_repo.compound_from_pubchem b on a.cid  = b.cid where a.cid != 0 and b.cid is null group by cid order by cid `)
